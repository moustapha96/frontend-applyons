...
const TopBar = ({ collapsed, setCollapsed }) => {
  ...
  const profileItems = [
    {
      key: "profile",
      label: <Link to="/institut/profil">{t('institutMenu.layout.topbar.mon_profil')}</Link>,
      icon: <UserOutlined />,
    },
    {
      key: "settings",
      label: <Link to="/institut/parametres">{t('institutMenu.layout.topbar.paramettre')}</Link>,
  ...
  const notificationItems = [
    ...
    },
    {
      ...
      label: (
        <div>
          <Text strong>Rappel</Text>
          <div>Demandes en attente depuis plus de 48h</div>
        </div>
      ),
    },
    {
