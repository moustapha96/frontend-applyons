...
import { getDossierAtraiterDetails } from "@/services/institutTraducteurService"
import { useAuthContext } from "@/context"

const { Text } = Typography

const InstitutTraducteurDossierATraiterDetails = () => {
    const { id } = useParams()
    ...
    useEffect(() => {
        const fetchDemande = async () => {
            try {
                const data = await getDossierAtraiterDetails(institut.id, id)
                setDemande(data)
            } catch (err) {
                setError(err.message)
                message.error(t("institutTraducteur.error_fetching_details"))
            } finally {
                setLoading(false)
            }
        }
    ...
    const formatPeriod = (periode) => {
        const periodMap = {
            "Printemps": t("institutTraducteur.periode_1"),
            "Été": t("institutTraducteur.periode_2"),
            "Automne": t("institutTraducteur.periode_3"),
            "Hiver": t("institutTraducteur.periode_4"),
            "Autre": t("institutTraducteur.periode_5")
        }
    ...
    const handleViewDocument = async (docId) => {
        try {
            ...
            message.error(t("institutTraducteur.error_loading_document"))
        } finally {
            setPdfLoading(false)
        }
    }
    ...
    return (
        <div style={{ background: "#f0f2f5", minHeight: "100vh" }}>
            <InstitutTraducteurBreadcrumb title={t("institutTraducteur.request_details")} SubTitle={demande?.code} />
                    ...
                    <Space direction="vertical" size="large" style={{ width: "100%" }}>
                        ...
                        <Card
                            ...
                            title={
                                <Space>
                                    <FileOutlined />
                                    <span>{t("institutPartageDetails.shared_documents")}</span>
                                </Space>
                            }
                        >
            ...
            <Modal
                title={t("institutTraducteur.pdf_document")}
                open={pdfVisible}
                onCancel={() => {
                    setPdfVisible(false)
                    setPdfData(null)
            ...
            >
                {pdfLoading ? (
                    <div style={{ textAlign: "center", padding: "20px" }}>
                        {/* <Spin tip={t("institutTraducteur.loading_document")} /> */}
                        <Spin />
                    </div>
                ) : pdfError ? (
                    <div style={{ textAlign: "center", padding: "20px" }}>
                    ...
                    <object
                        ...
                        aria-label={t("institutTraducteur.pdf_viewer")}
                    >
                        <iframe
                            src={`data:application/pdf;base64,${pdfData}`}
                            width="100%"
                            height={isSmallScreen ? "300px" : "500px"}
                            title={t("institutTraducteur.pdf_viewer")}
                        />
                    </object>
                ) : (
                    <div style={{ textAlign: "center", padding: "20px" }}>
                        <Text type="warning">{t("institutTraducteur.no_document_to_display")}</Text>
                    </div>
                )}
            </Modal>
        </div>
...
export default InstitutTraducteurDossierATraiterDetails
