...
const AdminProfil = () => {
    ...
    const handleImageUpload = useCallback(async (event) => {
        ...
        if (!allowedTypes.includes(file.type)) {
            toast.error(t("adminProfil.unsupported_file_type"));
            return;
        }

        const maxSize = 5 * 1024 * 1024; // 5MB
        if (file.size > maxSize) {
            toast.error(t("adminProfil.image_too_large"));
            return;
        }
        ...
        try {
            ...
            if (response && response.imageUrl) {
                ...
                toast.success(t("adminProfil.profile_image_updated"));
            } else {
                throw new Error(response.error || t("adminProfil.error_uploading_image"));
            }
        } catch (error) {
            console.error(t("adminProfil.error_uploading_image"), error);
            toast.error(t("adminProfil.error_uploading_image_retry"));
        } finally {
    ...
    const handleUpdateAdmin = async (e) => {
        ...
        if (adminUser.newPassword && adminUser.newPassword !== adminUser.confirmNewPassword) {
            toast.error(t("adminProfil.passwords_do_not_match"));
        ...
        if (adminUser.newPassword) {
            updateData['password'] = adminUser.password;
            updateData['newPassword'] = adminUser.newPassword;
        }

        const response = await modifyAccount(adminUser.id, updateData);
        // //console.log(response);
        if (response.error) {
            setError(response.error);
            setIsSubmitting(false);
            toast.error(t("adminProfil.error_updating_profile"));
            return;
        }
        toast.success(t("adminProfil.profile_updated"));
        setIsSubmitting(false);
    };
    ...
    const handleToggleUserStatus = async (userId, currentStatus) => {
        setIsToggled(true);
        try {
            //console.log(userId, 'isActive', !currentStatus);
                                ...
                                <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                                    ...
                                    <div className="mb-6">
                                        <div className="flex border-b border-gray-200">
                                            <button
                                                className={`py-2 px-4 font-medium ${activeTab === 'update' ? 'text-blueLogo border-b-2 border-blueLogo' : 'text-gray-500 hover:text-gray-700'}`}
                                                onClick={() => setActiveTab('update')}
                                            >
                                                {t("adminProfil.update_profile")}
                                            </button>
                                            <button
                                    ...
                                    {activeTab === 'update' && (
                                        <div className="bg-white shadow-md rounded-lg p-6">
                                            <h2 className="text-2xl font-semibold mb-4">{t("adminProfil.admin_profile")}</h2>
                                            <div className="flex items-center justify-center mb-6">
                                                <div className="relative">
                                                    <img
                                                        src={profileImage ? `data:image/jpeg;base64,${profileImage}` : avatar}
                                                        alt={t("authentic_page_adn")}
                                                        width={100}
                                                        height={100}
                                            ...
                                            <form onSubmit={handleUpdateAdmin} className="space-y-4">
                                                ...
                                                <div>
                                                    <label htmlFor="current-password" className="block text-sm font-medium text-gray-700 mb-1">
                                                        {t("adminProfil.current_password")}
                                                    </label>
                                                    <div className="relative">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e) => setAdminUser({ ...adminUser, newPassword: e.target.value })}
                                                            className="pl-10 w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                        />
                                                        <button
                                                            type="button"
                                                ...
                                                <div>
                                                    <label htmlFor="confirm-new-password" className="block text-sm font-medium text-gray-700 mb-1">
                                                        {t("adminProfil.confirm_new_password")}
                                                    </label>
                                                    <div className="relative">
                                                        <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
                                                        <input
                                                            ...
                                                            type="password"
                                                            value={adminUser.confirmNewPassword}
                                                            onChange={(e) => setAdminUser({ ...adminUser, confirmNewPassword: e.target.value })}
                                                            className="pl-10 w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                        />
                                                    </div>
                                                </div>
                                                <button
                                                    ...
                                                    className="w-full flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blueLogo hover:bg-rougeLogo focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
                                                >
                                                    {!isSubmitting ? <>
                                                        <Save className="mr-2" size={20} />
                                                    </> :
                                                        <Loader span className="mr-2 animate-spin" ></Loader>}
                                                    {isSubmitting ? t("adminProfil.saving") : t("adminProfil.save_changes")}
                                                </button>
                                            </form>
                                        </div>
                                    )}
