...
export default function DemandeurDocumentPartage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        const data = await getDocumentPartageDemandeur(demandeur.id)
        //console.log(data)
        setDocuments(data)
      } catch (err) {
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentDocuments && currentDocuments.map((document) => (
                      <tr key={document.id} className="hover:bg-gray-50 transition duration-300">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <Link
                            to={`/demandeur/institut/${document.institut.id}/details`}
                          >
                            <strong> {document.institut.name}</strong>
                          </Link>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <Link
                            to={`/demandeur/partages/${document.demande.id}/details`}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <Link
                            to={`/demandeur/document-partage/${document.id}/details`}
                            className="text-blueLogo hover:text-blueClaire mr-4 border border-blueLogo rounded-md px-2 py-1"
                          >
                            {t('demandeur.documentDetails')}
                          </Link>
                        </td>
                      </tr>
                    ))}
